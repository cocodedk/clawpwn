...
class ReportGenerator:
    ...
    def generate(self, config: Optional[ReportConfig] = None) -> Path:
        """
        ...
        findings = (
            self.session.session.query(Finding).filter_by(project_id=project.id).all()
        )
...
"""
        ...
        for severity in severity_order:
            if by_severity[severity]:
                html_content += (
                    f"<h3>{severity.upper()} ({len(by_severity[severity])})</h3>\n"
                )
...
"""
        ...
        report_file = (
            self.report_dir
            / f"pentest_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.html"
        )
...
"""
    ...
    def _generate_markdown(
        self, state, findings: List[Any], config: ReportConfig
    ) -> Path:
...
"""
        ...
        report_file = (
            self.report_dir
            / f"pentest_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
        )
...
"""
    ...
    def _generate_pdf(self, state, findings: List[Any], config: ReportConfig) -> Path:
        ...
        pdf_file = (
            self.report_dir
            / f"pentest_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf"
        )
...
def generate_report(
    project_dir: Path, format: str = "html", include_evidence: bool = True
) -> Path:
