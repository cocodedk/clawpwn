...
class CommandCompleter(Completer):
    ...
    COMMANDS: dict[str, list[str]] = {
        "scan": ["--depth", "--verbose", "-v", "--scanner", "-s", "--parallel", "-p", "--auto"],
        "target": [],
        "status": [],
        ...
        if prev in ("--depth", "-d"):
            for v in self.DEPTH_VALUES:
                if v.startswith(last_word):
                    yield Completion(v, start_position=-len(last_word))
            return
        ...
        if first == "config" and len(words) == 2:
            for a in self.CONFIG_ACTIONS:
                if a.startswith(last_word):
                    yield Completion(a, start_position=-len(last_word))
            return
        ...
        if last_word.startswith("-"):
            for opt in options:
                if opt.startswith(last_word):
                    yield Completion(opt, start_position=-len(last_word))
            return
